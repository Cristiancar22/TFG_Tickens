...
export const processTicket = async (req: AuthRequest, res: Response): Promise<any> => {
	try {
		...
		const ocrResponse = await axios.post('http://ocr_service:5000/ocr', form, {
		...
		const llmResponse = await axios.post<LlmResponse>('http://ticket-parser:5020/parse', { text });
		const parsed = llmResponse.data;
		console.log('✅ JSON estructurado:', JSON.stringify(parsed, null, 2));
		...
		return res.status(500).json({ message: 'Error al procesar el ticket' });
	}
};
